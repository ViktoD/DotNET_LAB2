...
namespace Lab2
{
    internal class Student : Person, IDateAndCopy
    {
        ...
        public Student() : this(person: new Person(), education: Education.Bachelor, group: 101)
        {
            ...
            Tests = new ArrayList() { new Test(), new Test("C++", false), new Test("Java", true) };
        }
        ...
        public string[] SampleSubject()
        {

            string[] sampleSubject;
            int index = -1;
            if(Exams!=null && Tests != null)
            {
                if(Exams.Count <= Tests.Count)
                {
                     sampleSubject = new string[Exams.Count];
                }

                else
                {
                     sampleSubject = new string[Tests.Count];
                }
                ...
                foreach(Exam exam in Exams)
                {
                    foreach(Test test in Tests)
                    {
                        ...
                        {
                            sampleSubject[++index] = exam.NameSubject;
                        }
                    }
                }
        ...
        public IEnumerator GetEnumerator () => new StudentEnumerator(SampleSubject());
    
        public IEnumerable PassedTestsAndExams()
        {

            if(AllGoodExams(2) != null)
            {
                foreach (object exam in AllGoodExams(2))
                {
                    yield return exam;
                }
            }
